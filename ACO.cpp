...
__global__ void workerAntKernel(
    ...
    int* tours,
    float* choice_info,
    float* selection_prob_all,  // [m][n], GLOBAL
    curandState* states
) {
    int thread_id = blockIdx.x * blockDim.x + threadIdx.x;
    if (thread_id >= m) return;

    extern __shared__ unsigned char shared_memory[]; // shared memory

    const int visited_ints = (n + 31) / 32;  // number of uint32_t needed
    uint32_t* visited = (uint32_t*)(shared_memory + threadIdx.x * visited_ints * sizeof(uint32_t));

    float* selection_prob = &selection_prob_all[thread_id * n_cities]; // global memory

    // Initialize visited array
    for (int i = 0; i < visited_ints; ++i) {
        visited[i] = 0;
    }

    int step = 0;
    int current_city = tours[thread_id * n + step];

    // Mark starting city as visited
    visited[current_city / 32] |= (1U << (current_city % 32));

    for (step = 1; step < n_cities; ++step) {
        float sum_probs = 0.0f;

        // Build selection_prob array
        for (int j = 0; j < n; ++j) {
            if (visited[j / 32] & (1U << (j % 32))) {
                selection_prob[j] = 0.0f;
            } else {
                float prob = choice_info[current_city * n + j];
                selection_prob[j] = prob;
                sum_probs += prob;
            }
        }

        // Roulette Wheel Selection
        float r = curand_uniform(&states[thread_id]) * sum_probs;
        float p = 0.0f;
        int selected_city = -1;
        for (int j = 0; j < n; ++j) {
            p += selection_prob[j];
        ...
        // Update tour and visited
         visited[selected_city / 32] |= (1U << (selected_city % 32));
